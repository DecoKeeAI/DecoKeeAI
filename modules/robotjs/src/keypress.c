...
/* Convenience wrappers around ugly APIs. */
	...
	#define WIN32_KEY_EVENT_WAIT_NEW(key, flags, delayGap) \
		(win32KeyEvent(key, flags), Sleep(delayGap))
#elif defined(USE_X11)
...
void toggleKeyCode(MMKeyCode code, const bool down, MMKeyFlags flags, int delayGap)
{
#if defined(IS_MACOSX)
	...
	if (down) {
		...
		if (delayGap == -1) {
            ...
            if (flags & MOD_META) WIN32_KEY_EVENT_WAIT_NEW(K_META, dwFlags, delayGap);
            if (flags & MOD_ALT) WIN32_KEY_EVENT_WAIT_NEW(K_ALT, dwFlags, delayGap);
            if (flags & MOD_CONTROL) WIN32_KEY_EVENT_WAIT_NEW(K_CONTROL, dwFlags, delayGap);
            if (flags & MOD_SHIFT) WIN32_KEY_EVENT_WAIT_NEW(K_SHIFT, dwFlags, delayGap);

		    WIN32_KEY_EVENT_WAIT_NEW(code, dwFlags, delayGap);
		}
	} else {
...
void tapKeyCode(MMKeyCode code, MMKeyFlags flags, int delayGap)
{
	toggleKeyCode(code, true, flags, delayGap);
	toggleKeyCode(code, false, flags, delayGap);
}

void toggleKey(char c, const bool down, MMKeyFlags flags, int delayGap)
{
...
#endif
	toggleKeyCode(keyCode, down, flags, delayGap);
}

void tapKey(char c, MMKeyFlags flags, int delayGap)
{
	toggleKey(c, true, flags, delayGap);
	toggleKey(c, false, flags, delayGap);
}
...
#else
	#define toggleUniKey(c, down) toggleKey(c, down, MOD_NONE, 5)
#endif
